...
const TimelineItem = ({ title, timeframe, description }: TimelineItemProps) => (
    ...
    return (
        <div className="text-white lg:w-[1024px] overflow-hidden pb-24 pl-8 pt-2">
            <ol className="relative border-l border-gray-200 dark:border-gray-700">
                {timelineData.map(({ title, timeframe, description }, index) => <TimelineItem title={title} timeframe={timeframe} description={description} />)}
            </ol>
        </div>
    )
}